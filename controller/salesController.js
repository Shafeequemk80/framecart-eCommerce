...
const loaddashboard = async (req, res) => {
  try {
    ...
    if (paymenttypevalue === 'weekly') {
      ({ filteredOrders, formattedLabels, dailySales, dailyOrderCounts } = await fetchWeeklyData());
    } else if (paymenttypevalue === 'monthly') {
      ({ filteredOrders, formattedLabels, dailySales, dailyOrderCounts } = await fetchMonthlyData());
    } else if (paymenttypevalue === 'yearly') {
      ({ filteredOrders, formattedLabels, dailySales, dailyOrderCounts } = await fetchYearlyData());
    }

    const totalSalesForMonth = calculateTotalSales(filteredOrders);
    const totalOrderCountForMonth = filteredOrders.length;

    const recentOrders = await Order.find().limit(5).sort({ orderDate: -1 }).populate('user').exec();
    ...
    console.log(paymentCounts); // Access total sales for the selected timeframe
...
console.log(weeklyOrders);
  ...
  const dailySales = Array.from({ length: 7 }, (_, i) => ({
    date: new Date(firstDayOfWeek.getFullYear(), firstDayOfWeek.getMonth(), firstDayOfWeek.getDate() + i),
    totalSales: 0,
  }));
  ...
  const formattedLabels = dailySales.map((day) => moment(day.date).format("ddd D"));


  return { filteredOrders: weeklyOrders, formattedLabels, dailySales, dailyOrderCounts };
};
...
const fetchMonthlyData = async () => {
  ...
  const dailyOrderCounts = Array.from({ length: lastDayOfMonth.getDate() }, (_, i) => 0);
  ...
  const dailySales = Array.from({ length: lastDayOfMonth.getDate() }, (_, i) => ({
  ...
  const formattedLabels = dailySales.map((day) => moment(day.date).format("MMM D"));

  return { filteredOrders: monthlyOrders, formattedLabels, dailySales, dailyOrderCounts };
  ...
  const formattedLabels = dailySales.map((month) => moment(`${currentYear}-${month.month}`, "YYYY-M").format("MMM"));
  

  return { filteredOrders: yearlyOrders, formattedLabels, dailySales, dailyOrderCounts:monthlyOrderCounts };
};
...
const loadsales =async (req, res) => {
  ...
  try {
    const paymenttypevalue = req.query.filter||"weekly";
    const Datestart  = new Date(req.query.startDate|| new Date().setDate(new Date().getDate() - 30));
...
console.log(endDate,moment(endDate).format('YYYY-MM-DD'));
    ...
    const totalSalesForMonth = monthlyOrders.reduce((total, order) => total + order.amount, 0);
    ...
    const formattedLabels = dailySales.map((day) => day.date.getDate() + ' ' + moment(day.date).format("MMM"));
...
res.render('salesreport',{data,totalData,
   order: orders,
...
const loadchart =async (req, res) => {
    ...
    const userData = await User.find()
      ...
      const formattedMonth = moment().month(currentMonth - 1).format("MMM");


      const totalSales = {
        labels: Array.from({ length: lastDayOfMonth.getDate() }, (_, i) => `${i + 1} ${formattedMonth}`),
       data: dailySales
          }
    ...
    const totalSale = totalSaleResult.length > 0 ? totalSaleResult[0].totalSale : 0;
  ...
  try {
    ...
    orderData.forEach((order) => {
      const formattedOrder = {
        ...
        'products.product.name': order.products.map((p) => p.product.productname).join('\n'),
        'products.count': order.products.map((p) => p.count).join('\n'),
        'products.totalprice': order.products.map((p) => p.totalprice).join('\n'),
        'products.paymentStatus': order.products.map((p) => p.paymentStatus).join('\n'),
        'products.orderStatus': order.products.map((p) => p.orderStatus).join('\n'),
